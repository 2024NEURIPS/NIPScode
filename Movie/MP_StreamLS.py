...
class MP_StreamLS():
    ...
    def MP_LS_algorithm(self):
        start_time = time.time()
        self.LS_algorithm()
        for i in range(2,self.passes+1):
            self.counter = 0
            for movie_id, group in zip(self.data_id,self.data_group):
                # if self.counter % 1000 == 0:
                #     print(self.counter," ",len(self.S)," ",self.func_value)
                #     result = [self.S_group.count(int(l)) for l in range(self.l)]
                #     print(result)
                # self.counter += 1
                ...
                if min_id == -1:
                    continue
                if self.is_element_add(min_id,movie_id,(1.+1./i) * min_value):
                    self.update_S(min_id,movie_id,group)

            # print(i,"_ops ",self.func_value)
    ...
    def main(self):
        self.data_deal()
        self.MP_LS_algorithm()
        file_name = "MP_StreamLS_" + "K=" + str(self.K) + "_movie_PR_output.txt"
...
def func():
    ...
    for k in k_list:
        mp_sls = MP_StreamLS(data,matrix_factorized,user_movie_matrix,k,uid)
        mp_sls.main()
